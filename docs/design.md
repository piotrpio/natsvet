...
- Known nats-server header list (`headerkey-near-miss`): every `Nats-*` string literal
  in the pinned nats-server's non-test `server/` sources (46 at v2.14.7, found by
  parsing, so comments do not count), generated by `tablegen -server` into
  `server_headers_table.go`. The corpus job regenerates it from its nats-server clone
  and fails on a difference. `Header` merges it with the nats.go table (a header only
  the server uses has no constants), and `KnownHeaders` lists the union.
...
- **Hooks**: methods `Get`, `Set`, `Add`, `Values`, `Del` on `nats.Header` (also reached
  via `jetstream.Msg.Headers()`) and `Get`, `Values` on `micro.Headers`; direct map
  indexing `h["..."]`; keys of a `nats.Header{...}` or `micro.Headers{...}` literal; and
  the operand compared (`==`, `!=`, `switch` cases) with the key variable of a
  `for k := range h` over a header that the loop body never assigns.
- **Detect**, over the known set (the nats.go `Nats-` table and the nats-server header
  list from §2.4):
  1. The key is a constant that equals a known header case-insensitively but not
     exactly.
  2. The key starts with `Nats-` (any case), is not a known header, and is within
     case-insensitive Levenshtein distance 2 of one (the nearest, the lexically first
     on a tie), or else begins with a known header followed directly by a letter or
     digit (the longest such header). `Nats-TTL-Seconds` and connect's `Nats-Has-More`
     are neither.
- **Message**: `header key "nats-msg-id" does not match "Nats-Msg-Id"; nats.go header
  lookups are case-sensitive`; `header key "Nats-TTLSeconds" is not a header NATS sets
  or reads; the closest NATS header is "Nats-TTL"`.
- **Fix**: for item 1, replace the literal with the qualified constant
  (`jetstream.MsgIDHeader`, `nats.MsgIdHdr`) when that package is already imported in
  the file; otherwise replace it with the correctly cased string literal. Prefer the
  constant from the package the file already uses; when both are imported prefer
  `jetstream`. Item 2 has no fix: the intended header is a guess.
- **FP**: user-defined headers outside the `Nats-` namespace are never touched; inside
  it, only keys one slip away from a known header are reported, and the corpus has
  none besides the two bugs (§8 item 9).
- **Tests**: `Get("nats-msg-id")` with `jetstream` imported (fix → constant), without
  (fix → literal), `Set("X-My-Header", v)` (ok), map index form, header literal keys,
  range-key comparisons and switches, near-misses and the silent `Nats-Has-More`,
  `Nats-X`, `Data-TTL`, `Nats-TTL-Seconds`; golden files for fixes.
...
8. **`headerkey-near-miss`** (done): §8 item 9 — near-miss `Nats-*` keys, the
   nats-server header list (`tablegen -server`, checked by the corpus job), and two new
...
9. ~~`headerkey` near-miss: an unknown `Nats-*` key within a small edit distance of a
   known header.~~ Done in `headerkey-near-miss` (§3.1 `headerkey` item 2), with a
   correction: `Nats-TTLSeconds` is distance 7 from `Nats-TTL`, so no small threshold
   reaches it, and distance 3 already reaches `Nats-X` from `Nats-TTL`. Two shapes
   replace the threshold: distance ≤ 2, and a known header with letters or digits
   appended. Of the four `Nats-*` literals in the corpus that are not NATS headers,
   they report the two bugs (natscli's `Nats-UpTo-Sequnce`, in a comparison with a
   header range key, and go-choria's `Nats-TTLSeconds`) and neither legitimate key
   (connect's `Nats-Has-More`, a go-choria test's `Nats-X`). Nothing reserves the
   `Nats-` prefix, so an unknown key alone is not a finding.
