...
| `headerkey` | on | yes | a header key that differs only in case from a NATS header (`nats-msg-id` vs `Nats-Msg-Id`), so the lookup silently fails, or that is one slip from one (`Nats-TTLSeconds` for `Nats-TTL`, no fix) |
