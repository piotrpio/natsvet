# rule-consumerconfig Specification
...
#### Scenario: Logged, then fixed, then submitted
- **WHEN** a function has `cfg := jetstream.ConsumerConfig{IdleHeartbeat: 5 * time.Second}`, then `log.Printf("%v", cfg)`, then `cfg.DeliverSubject = "d"`, then `CreateConsumer(ctx, "S", cfg)`
- **THEN** the rule reports nothing: the logger's `...any` parameter is not a hand-off, and the field is assigned before the real one
