...
- [x] 4.2 Confirm CI is green on the pushed candidate commit for both Go versions; verify with `gh run view`
- [x] 4.3 Verify the install path without a tag: `go install github.com/piotrpio/natsvet/cmd/natsvet@latest && natsvet -version` prints the pseudo-version of the pushed `main` and `natsvet -h` lists the eleven rules
