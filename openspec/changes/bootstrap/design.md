...
**legacyjs.** Walk `*ast.Ident` through `TypesInfo.Uses`. `*types.TypeName` in the `nats` package whose name is in the table → report at the ident. `*types.Func`: with a receiver, key `Recv.Method`; without, key `Name`; report when in the table. Interface method calls resolve to the `*types.Func` of the interface that declares the method, so `js.Publish` on a `nats.JetStreamContext` keys as `JetStream.Publish` (the embedded interface), and the generator records interface methods under their declaring interface for the same reason. One diagnostic per ident, so a composite literal type reports once and an option constructor passed to a legacy call reports separately from the call, as the spec requires.
