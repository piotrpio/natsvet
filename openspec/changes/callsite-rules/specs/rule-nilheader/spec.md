...
nilheader reports Header.Set or Header.Add on a nats.Msg built as a composite literal without a Header. nats.Header.Set and Add are plain map writes, nats.NewMsg allocates the map and a literal does not, so the call panics with an assignment to a nil map.
