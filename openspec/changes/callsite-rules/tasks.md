...
- [ ] 2.1 Write `testdata/duration/{jetstream,legacy,micro}.go` with one `// want` line per scenario in `rule-duration/spec.md` (Request, Timeout option, AckWait field, MaxAge legacy, NextMsg, BackOff elements, named constant, arithmetic, typed unit, zero/negative, large raw, conversion, time.Sleep, int parameter); verify the test fails before the analyzer exists
- [ ] 2.2 Implement `analyzers/duration` with type-driven parameter and field discovery and the untyped-constant AST test; verify the analysistest passes
- [ ] 2.3 Register `duration` in `Analyzers()`; verify `bin/natsvet ./duration/` from `testdata` prints the expected diagnostics

## 3. Rule subject

- [ ] 3.1 Write `testdata/subject/{core,jetstream,legacy,micro}.go` with one `// want` line per scenario in `rule-subject/spec.md` (each invalidity reason, reply argument, NewMsg, Msg literal fields, micro endpoint, wildcard publish vs subscribe, look-alike tokens, concatenated constants, non-constant); verify the test fails before the analyzer exists
- [ ] 3.2 Implement `analyzers/subject` with the hook table, `subjectInvalid` reasons in nats.go/server order, and the publish-literal check; verify the analysistest passes
- [ ] 3.3 Register `subject` in `Analyzers()`; verify the binary prints the expected diagnostics on `testdata/subject`

## 4. Rule ctxdeadline

- [ ] 4.1 Write `testdata/ctxdeadline/ctxdeadline.go` with one `// want` line per scenario (Background/TODO to each of the four methods, variable, derived context, jetstream exempt); verify the test fails before the analyzer exists
...
- [ ] 6.1 Write `testdata/nilheader/nilheader.go` with one `// want` line per scenario (pointer and value literal with Set/Add, NewMsg, literal with Header, Header assigned later, nil-safe methods, parameter, reassigned); verify the test fails before the analyzer exists
- [ ] 6.2 Implement `analyzers/nilheader` on `SingleDefinition` and `CompositeFields`; verify the analysistest passes and register it in `Analyzers()`

## 7. Rule drain

- [ ] 7.1 Write `testdata/drain/drain.go` with one `// want` line per scenario (adjacent, `if err := Drain()` form, `err := Drain()` + check form, wait between, different connections, deferred Close); verify the test fails before the analyzer exists
- [ ] 7.2 Implement `analyzers/drain` as a statement-list walk with the single tolerated `if`; verify the analysistest passes and register it in `Analyzers()`
